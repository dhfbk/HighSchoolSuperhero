...
//surface calculations
	...
	void SurfaceColor(out fixed3 albedo, out fixed alpha, float2 uv)
	{
		...
		//fixed4 col = tex2D(_MainTex, i.uv + float2(sin(i.vertex.y/25 + _Time[3]*2)/100, sin(i.vertex.x/25 + _Time[3]*2)/100)) * _Color;
		albedo = c.rgb;
		alpha = _Transp;
		
	}

	void SurfaceColorAnimated(out fixed3 albedo, out fixed alpha, VertexOutputForward o)
		{
			fixed4 c = tex2D(_MainTex, o.uv_Main+ float2(sin((o.posWorld.y+o.posWorld.z) + _Time[3]*2)/100, sin(o.posWorld.y+o.posWorld.z + _Time[3]*2
			)/100)*_WavySpeed) * _Color;
		    albedo = c.rgb;
			alpha = _Transp;
		}
	...
	//only include initsurface when not meta pass
		...
		)
		{
			...
			//init surface color
			SurfaceColorAnimated(mkts.Color_Albedo, mkts.Alpha, o);
